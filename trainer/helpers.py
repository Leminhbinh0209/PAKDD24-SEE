...
def evaluate(get_emb_f, ds_name):
    if ds_name != "Inshop":
        ...
        recall_head = get_recall(*emb_head, *emb_head, ds_name)
        
    else:
        emb_head_query = get_emb_f(ds_type="query")
        emb_head_gal = get_emb_f(ds_type="gallery")
        recall_head = get_recall(*emb_head_query, *emb_head_gal, ds_name)
    return recall_head
...
def get_recall(xq, yq, index_q, xg, yg, index_g, ds_name):
    if ds_name == "SOP":
    ...
    # print(" | ".join([f"R@{u}:{100*v:.2f}" for (u,v) in zip(k_list, recall)]))
    return recall
